...
dataverificacao obterDataAtual(void) {
    ...
    dataverificacao d = { t->tm_mday, t->tm_mon + 1, t->tm_year + 1900 };
    return d;
}
...
equipamento *criarEquipamento(int *proxCIU) {
    ...
    novo->CIU = (*proxCIU)++;
    printf("\n[CIU: %d]\n", novo->CIU);
    lerTexto("Nome: ",        temp, MAXCHAR); novo->nome       = duplicarString(temp);
    lerTexto("Tipo: ",        temp, MAXCHAR); novo->tipo       = duplicarString(temp);
    lerTexto("Marca: ",       temp, MAXCHAR); novo->marca      = duplicarString(temp);
    lerTexto("Modelo: ",      temp, MAXCHAR); novo->modelo     = duplicarString(temp);
    lerTexto("MAC: ",         temp, MAXCHAR); novo->mac        = duplicarString(temp);
    lerTexto("IP: ",          temp, MAXCHAR); novo->IP         = duplicarString(temp);
    lerTexto("Localizacao: ", temp, MAXCHAR); novo->localizacao= duplicarString(temp);
...
void listar(equipamento *cabeca) {
    ...
    printf("\n=== Equipamentos ===\n");
    while (cabeca) { imprimirEquipamento(cabeca); cabeca = cabeca->prox; }
}
...
void removerEquipamento(equipamento **cabeca) {
    int ciu, conf;
    printf("CIU a remover: "); scanf("%d", &ciu); getchar();
...
void alterarEquipamento(equipamento *cabeca) {
    ...
    do {
        ...
        switch (op) {
            case 1: lerTexto("Nome: ",        temp, MAXCHAR); free(e->nome);       e->nome       = duplicarString(temp); break;
            case 2: lerTexto("Tipo: ",        temp, MAXCHAR); free(e->tipo);       e->tipo       = duplicarString(temp); break;
            case 3: lerTexto("Marca: ",       temp, MAXCHAR); free(e->marca);      e->marca      = duplicarString(temp); break;
            case 4: lerTexto("Modelo: ",      temp, MAXCHAR); free(e->modelo);     e->modelo     = duplicarString(temp); break;
            case 5: lerTexto("IP: ",          temp, MAXCHAR); free(e->IP);         e->IP         = duplicarString(temp); break;
            case 6: lerTexto("MAC: ",         temp, MAXCHAR); free(e->mac);        e->mac        = duplicarString(temp); break;
            case 7: lerTexto("Localizacao: ", temp, MAXCHAR); free(e->localizacao);e->localizacao= duplicarString(temp); break;
            case 8: free(e->estado); e->estado = perguntarEstado(); break;
            case 9: printf("Nova data:\n"); lerData(&e->Data.dia, &e->Data.mes, &e->Data.ano); break;
            case 0: break;
...
void testarConectividade(equipamento *cabeca, int ciu, int so) {
    ...
    int ok = analisarResultadoPing(so);
